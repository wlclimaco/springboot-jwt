...
(function() {
	...
	function CarouselController($timeout, $attrs, $interval, $window) {
		...
		that.updateSlidesContainerWidth = function() {
			...
			that.resizeSlides();
			var newSlidesContainerWidth = that.currentWidth
					* that.cloneData.length;
		...
		that.scaleContent = function() {
			...
			var currentElementParentWidth = that.elementParent
					.prop('offsetWidth');
			if (currentElementParentWidth < that.maxWidth) {
				console.log(currentElementParentWidth, that.maxWidth);
				var newHeight = (that.maxHeight * currentElementParentWidth)
						/ that.maxWidth;
		...
		that.scaleMarginLeft = function(newSlidesContainerWidth) {
			if (that.currentSlidesContainerWidth
					&& that.currentSlidesContainerWidth !== newSlidesContainerWidth) {
				that.currentMarginLeftValue = that.currentMarginLeftValue
		...
		that.restartFromLastItem = function() {
			$timeout(
					function() {
						that.disableTransitions();
						that.currentMarginLeftValue = (that.currentWidth * that.data.length)
								* -1;
						that.applyMarginLeft();
		...
		that.applyMarginLeft = function() {
			that.slidesContainer.css('margin-left', that.currentMarginLeftValue
					+ 'px');
		};
...
(function() {
	...
	angular.module('jkAngularCarousel').directive('jkCarousel',
			[ CarouselDirective ]);
